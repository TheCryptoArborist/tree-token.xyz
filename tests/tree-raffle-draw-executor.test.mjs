...
test('Knowledge Trial settlement finalizes the random draw before registering a new award', async () => {
  const wallet = `0x${'3'.repeat(64)}`;
  const snapshot = {
    roundId: 'knowledge:2026-09-19',
    onchainDrawId: 'knowledge:2026-09-19:award',
    resolutionCommitment: 'ab'.repeat(32),
    ledgerCommitment: 'ab'.repeat(32),
    totalTickets: '1',
    wallet,
  ...
  const chain = Object.create(SuiDailyDrawChain.prototype);
  chain.readPrize = async () => null;
  chain.readDraw = async () => null;
  chain.executeDraw = async () => {
    calls.push('draw');
    return { digest: '1'.repeat(40) };
  };
  chain.registerWinner = async (_snapshot, winner, amountRaw) => {
    calls.push('register');
    return { digest: '2'.repeat(40), winner, amountRaw };
  ...
  assert.deepEqual(calls, ['draw', 'register']);
});
