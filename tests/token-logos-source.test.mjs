...
const swapScript = readFileSync(new URL('../dapp/swap-router.js', import.meta.url), 'utf8');
...
test('swap, limit, and V3 token direction changes preserve logo presentation', () => {
  assert.match(swapScript, /inputIcon\.src = inputSymbol === 'SUI' \? '\.\.\/assets\/sui-token\.svg' : '\.\.\/assets\/tree-token\.png'/);
  assert.match(swapScript, /outputIcon\.src = outputSymbol === 'SUI' \? '\.\.\/assets\/sui-token\.svg' : '\.\.\/assets\/tree-token\.png'/);
  assert.match(swapStyles, /swap-token-icon\.sui/);
  assert.match(swapStyles, /swap-token-icon\.tree/);
