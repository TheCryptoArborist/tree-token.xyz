...
import { SUI_TYPE, TREE_TYPE, TURBOS_SUI_TREE_FEE_TYPE, TURBOS_SUI_TREE_POOL, normalizeTreeSwapQuote, validateSwapRequest } from '../netlify/lib/tree-swap-route.ts';
...
const buyTurbos = { type: 'direct', venue: 'turbos', venueLabel: 'Turbos', executionKind: 'turbos-direct', pairId: TURBOS_SUI_TREE_POOL, tokenIn: SUI_TYPE, tokenOut: TREE_TYPE, amountIn: '1000000000', amountOut: '35500000000', minAmountOut: '35145000000', priceImpactPercent: 0.2, priceImpactTier: 'low', gasEstimate: '0', feePercent: 1, coinAType: TREE_TYPE, coinBType: SUI_TYPE, feeType: TURBOS_SUI_TREE_FEE_TYPE, aToB: false, nextTickIndex: -35754 };
const buy = normalizeTreeSwapQuote({ bestRoute: buyV2, directRoutes: [buyV2, buyV3], buildMetadata: { hops: [{ pairId: buyV2.hops[0].pairId, minAmountOut: '35052005454' }] } }, { ...buyRequest, generatedAt: '2026-08-12T20:58:46.137Z' }, [buyTurbos]);
assert.equal(buy.selectedRoute.venue, 'turbos');
assert.equal(buy.selectedRoute.amountOut, '35500000000');
assert.equal(buy.selectedRoute.minAmountOut, '35145000000');
assert.equal(buy.decimalsIn, 9);
assert.equal(buy.decimalsOut, 6);
assert.equal(buy.routes.length, 3);
...
assert.throws(() => normalizeTreeSwapQuote({}, buyRequest, [{ ...buyTurbos, pairId: '0x0' }]), /No allowlisted/);
console.log('TREE best-route quote normalization: PASS (SuiDex V2/V3 + allowlisted direct Turbos route)');
