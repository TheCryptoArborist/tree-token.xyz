...
assert.doesNotMatch(earn, /Kelpie/i, 'The V2 Earn section must not list the Kelpie V3 vault.');
assert.match(v3Workspace, /Managed V3 Option — Kelpie/);
assert.match(v3Workspace, /Automated SUI \/ TREE range management/);
assert.ok(v3Workspace.includes(`href="${KELPIE_TREE_VAULT}"`), 'V3 must link to the verified Kelpie TREE SuiDex V3 vault.');
const externalEarnLinks = [...earn.matchAll(/<a\b[^>]*href="([^"]+)"[^>]*target="_blank" rel="noopener noreferrer"[^>]*>/g)].map((match) => match[1]);
assert.equal(externalEarnLinks.length, 1, 'Earn may expose only the V2 position explorer as an external link.');
assert.deepEqual(externalEarnLinks.filter((href) => href.startsWith('https://kelpie.network/')), [], 'Earn must not link to Kelpie because Kelpie is a V3 route.');
...
console.log('Earn routes source: PASS (Earn remains V2-only; verified Kelpie managed route and native controls remain in V3)');
