...
`npm run verify:production` and `npm run build` verify/build the 189-file published baseline in `dist/`. `npm run build:preview` builds a separate 191-file candidate in `dist-preview/`, using public manifest paths (including supplementary assets), four explicit frontend overlays, and two explicit STI modules. `tests/sti-preview-build.test.mjs` checks every output digest and the complete candidate inventory.
