...
`npm run build` produces the unchanged published snapshot in `dist/`. `npm run build:preview` produces the native STI candidate in `dist-preview/`, overlaying four reviewed frontend files and adding three STI modules and the local STI icon. The manifest retains published digests; archived baseline copies supply those four files. Deploy previews use only `netlify/preview-functions`, a public GET-only proxy without background jobs or writes. A review preview is not a production release package.
