...
export default async (request: Request) => {
  ...
  if (!apiKey) {
    return Response.json({
      ...
      coverage: LEADERBOARD_COVERAGE, holderCount: null, displayedCount: 0, excludedCount: 0, sharedProtocolExcludedCount: 0,
      exclusionCoverage: null, entries: [],
  ...
  try {
    ...
    return Response.json({
      ...
      displayedCount: result.displayedCount, excludedCount: result.excludedCount,
      sharedProtocolExcludedCount: result.sharedProtocolExcludedCount,
      exclusionCoverage: result.exclusionCoverage, entries: result.entries, warnings: result.warnings,
