...
const approved='8ac47c26e96596307d4b8b77764a4bdb19ac7214';
const baselineId='6ab345e1b9c190b8487f1d7b';
const candidatePath='.netlify/sti-wallet-release-candidate.json';
const statePath='.netlify/sti-wallet-production-release.json';
const changes=['/dapp/index.html','/scripts/wallet.js','/dapp/sti-widget.js'];
...
const identities=functions=>functions.map(f=>{
  // Netlify materializes formerly omitted defaults on reused functions. Both
  // deployments must explicitly report cmh below; retain strict package/provider
  // identities and compare the documented effective defaults, not key presence.
  // https://docs.netlify.com/build/functions/configuration/ (1024 MB, cmh/Ohio)
  const fields={...f,m:f.m??f.mo??1024,rg:f.rg??f.rgo??'us-east-2'};
  return Object.fromEntries(['n','d','id','oid','ro','p','m','rg','r','im','dn','g','bd','s','a','c'].filter(key=>fields[key]!=null).map(key=>[key,fields[key]]));
...
assert.equal(live.functions_region,'cmh','Do not infer function region defaults outside the verified Ohio deployment');
...
if(mode==='plan'){
  ...
  for(const pathname of changes){
    ...
    const reviewedHash=sha(execFileSync('git',['show',`${approved}:${source}`]));
...
// All deployed bytes are pinned below; scratch QA artifacts are never uploaded.
git('diff','--exit-code',approved,'--','dapp/index.html','scripts/wallet.js','dapp/sti-widget.js','production/manifest.json');
...
for(const pathname of changes)assert.equal(sha(fs.readFileSync(pathname.slice(1))),sha(execFileSync('git',['show',`${approved}:${pathname.slice(1)}`])));
...
if(mode==='preview'){
  ...
  const deploy=await api(`sites/${siteId}/deploys?title=STI%20wallet%20approval%20fix%20PR35%20preserved%20backend`,'POST',{files:expected,functions,functions_config,function_schedules:live.function_schedules,draft:false,async:false,branch:live.branch,commit_ref:commit});
...
assert.equal(ready.functions_region,live.functions_region);
...
fs.writeFileSync('.netlify/sti-wallet-production-report.json',JSON.stringify(report,null,2)+'\n');
fs.writeFileSync('.netlify/sti-wallet-production-deploy.json',JSON.stringify(ready,null,2)+'\n');
