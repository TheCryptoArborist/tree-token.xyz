...
for(const name of ['tree-volume','tree-liquidity']){
 ...
 const bytes=fs.readFileSync(result.path);const {name:builtName,runtimeVersion,runtimeAPIVersion,buildData,invocationMode,routes}=result;packages.push({name:builtName,runtimeVersion,runtimeAPIVersion,buildData,invocationMode,routes,path:'production/metrics-functions/'+name+'.zip',sha256:crypto.createHash('sha256').update(bytes).digest('hex')});
}
